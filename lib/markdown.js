...
function renderPdf(label, body) {
  ...
  const title = label || lines[1] || "PDF 文档";
  const note = lines.length > 1 && !lines[1].startsWith("http") ? lines[1] : "";
  return (
    `<figure class="rich pdf-block">` +
    `<div class="pdf-head">` +
    `<span class="pdf-icon" aria-hidden="true">PDF</span>` +
    `<div><p class="pdf-title">${escapeHtml(title)}</p>` +
    (note ? `<p class="pdf-note">${escapeHtml(note)}</p>` : "") +
    `</div>` +
    `<span class="pdf-actions"><a class="btn" href="${escapeHtml(url)}" target="_blank" rel="noopener">打开</a><a class="btn btn-gold" href="${escapeHtml(url)}" download>下载</a></span>` +
    `</div>` +
    `<iframe class="pdf-frame" src="${escapeHtml(url)}" title="${escapeHtml(title)}" loading="lazy"></iframe>` +
    `</figure>`
  );
}
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith(":::")) {
      ...
      if (kind === "quote") {
        ...
        out.push(renderPdf(label, body.join("\n")));
      } else {
